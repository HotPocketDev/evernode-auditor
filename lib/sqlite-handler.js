...
class SqliteDatabase {
        ...
        if (filter) {
            ...
            for (const columnName of columnNames) {
                if (filter[columnName]) {
                    ...
                    filterStr += `${columnName} IS NULL AND `;
                }
            }
        }
    ...
    getValuesIn(tableName, filter) {
        ...
        if (filter) {
            const columnNames = Object.keys(filter);
            for (const columnName of columnNames) {
                filterStr += `${columnName} IN (${Array(filter[columnName].length).fill('?').join(', ')}) AND `;
                values = values.concat(filter[columnName]);
            }
        }
        filterStr = filterStr.slice(0, -5);
    ...
    async updateValuesIn(tableName, value, filter) {
        if (!this.db)
            throw 'Database connection is not open.';
